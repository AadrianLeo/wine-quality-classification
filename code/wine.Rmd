...
plot(
  ...
  lwd = 2,                  # thicker line
  main = "ROC Curve for SVM (Validation Set)",
  legacy.axes = TRUE
)
...
```
# (c) Fair Comparison of Logistic Regression vs SVM (Test Set)

```{r compare-test}

# --- Logistic Regression: compute test predictions + metrics using best_thr ---
# (These variables already exist from your LR section: test_prob, best_thr, roc_test)
logit_test_prob <- test_prob
logit_thr <- best_thr

logit_test_pred <- factor(
  ifelse(logit_test_prob >= logit_thr, "Premium", "Poor"),
  levels = c("Poor", "Premium")
)

cm_logit_test <- confusionMatrix(logit_test_pred, test$quality_binary, positive = "Premium")

# AUC for Logistic Regression (already computed as auc(roc_test), but we store it)
auc_logit_test <- as.numeric(auc(roc_test))


# --- SVM: compute metrics using best_thresh ---
# (These variables already exist from your SVM section: svm_test_prob, best_thresh, svm_roc_test)
svm_test_pred2 <- factor(
  ifelse(svm_test_prob >= best_thresh, "Premium", "Poor"),
  levels = c("Poor", "Premium")
)

cm_svm_test <- confusionMatrix(svm_test_pred2, test$quality_binary, positive = "Premium")
auc_svm_test <- as.numeric(auc(svm_roc_test))


# --- Create a fair side-by-side table (Premium is the positive class) ---
compare_table <- data.frame(
  Model = c("Logistic Regression", "SVM (Radial)"),
  Test_AUC = c(auc_logit_test, auc_svm_test),
  Precision_Premium = c(cm_logit_test$byClass["Precision"], cm_svm_test$byClass["Precision"]),
  Recall_Premium    = c(cm_logit_test$byClass["Recall"],    cm_svm_test$byClass["Recall"]),
  F1_Premium        = c(cm_logit_test$byClass["F1"],        cm_svm_test$byClass["F1"]),
  Balanced_Accuracy = c(cm_logit_test$byClass["Balanced Accuracy"], cm_svm_test$byClass["Balanced Accuracy"])
)
...
**Test-set comparison :**  
The table compares Logistic Regression and the Radial SVM on the same held-out test set using ROC-AUC and threshold-based metrics focused on the minority class (*Premium*). Logistic Regression achieves a slightly higher **Test AUC** (0.826) than SVM (0.807), indicating better overall ranking/discrimination across thresholds. Using the validation-selected thresholds, both models reach the same **Recall for Premium** (0.907), meaning they successfully identify most Premium wines. However, **Precision** is low for both models (~0.23), showing that many wines predicted as Premium are actually Poor, which is expected in an imbalanced setting. The SVM shows a marginally higher **F1-score** (0.373 vs 0.370) and **Balanced Accuracy** (0.723 vs 0.720), but the differences are very small. Overall, Logistic Regression is preferable if we prioritize global discrimination (AUC), while SVM offers a slight advantage in threshold-based classification metrics; given the minimal gap, model choice can be guided by the preferred objective and interpretability considerations.

# Model-Agnostic XAI for Logistic Regression and SVM

```{r xai-setup}
# install.packages("DALEX")        # run once if needed
# install.packages("ingredients")  # optional, DALEX imports it

library(DALEX)

# Use same training data as for the models
x_train <- train %>% dplyr::select(-quality_binary)
y_train <- ifelse(train$quality_binary == "Premium", 1, 0)

# Explainer for Logistic Regression (probability of Premium)
explainer_logit <- explain(
  model            = logit_model,
  data             = x_train,
  y                = y_train,
...
# Explainer for SVM (caret::train with svmRadial, probability of Premium)
explainer_svm <- explain(
  ...
  label            = "SVM (Radial)",
  predict_function = function(m, newdata) {
    predict(m, newdata, type = "prob")[, "Premium"]
  }
)
```

# Global permutation feature importance for both models
```{r feature-importance}
# Custom binary cross-entropy loss for numeric 0/1 target and probability predictions
loss_bce <- function(observed, predicted) {
  eps <- 1e-15
  predicted <- pmin(pmax(predicted, eps), 1 - eps)
  -mean(observed * log(predicted) + (1 - observed) * log(1 - predicted))
}

fi_logit <- model_parts(
  explainer_logit,
...
# Side-by-side comparison (top variables)
plot(fi_logit, fi_svm, max_vars = 10)
fi_logit
fi_svm
```

# Choose a few key variables for detailed effect plots
```{r pdp-ale}
vars_of_interest <- c("alcohol", "sulphates", "volatile.acidity")

# Partial dependence profiles
pdp_logit <- model_profile(
  explainer_logit,
...
# (Alternative: ALE profiles instead of PDPs)
ale_logit <- model_profile(
  explainer_logit,
  variables = vars_of_interest,
  type      = "accumulated"
)

ale_svm <- model_profile(
  explainer_svm,
  variables = vars_of_interest,
  type      = "accumulated"
)
...
plot(ale_svm)
```
# Microscopic (Local) Explanations for Individual Wines
```{r local-explanations}
set.seed(123)
```

```{r select-local}
example_idx <- sample(seq_len(nrow(test)), 3)
local_obs   <- test[example_idx, ]
local_x     <- local_obs %>% dplyr::select(-quality_binary)
local_y     <- local_obs$quality_binary
local_obs
local_y
```

# Break-down (additive) explanations for Logistic Regression
```{r break-down}
# Break-down (additive) explanations for Logistic Regression
bd_logit <- predict_parts(
  explainer_logit,
  new_observation = local_x[1, , drop = FALSE],
  type            = "break_down"
)

# Break-down explanations for SVM
...
# SHAP-like local explanations (can be slower)
shap_logit <- predict_parts(
  explainer_logit,
  new_observation = local_x[1, , drop = FALSE],
  type            = "shap"
)

shap_svm <- predict_parts(
  explainer_svm,
  new_observation = local_x[1, , drop = FALSE],
  type            = "shap"
)
...
- Iowa State University Extension. (2021). [Understanding Wine Quality Parameters](https://www.extension.iastate.edu/wine/wp-content/uploads/2021/09/FS40.pdf).
- Cortez, P. (2018). [Modeling wine preferences by data mining from physicochemical properties](https://www.nature.com/articles/s41598-018-19185-5). *Scientific Reports*.
- Vinetur. (2025). [Sulfur in winemaking: Industry divided over impact on taste and quality](https://www.vinetur.com/en/2025012084084/sulfur-in-winemaking-industry-divided-over-impact-on-taste-and-quality.html).
